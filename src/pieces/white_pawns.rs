...
pub fn moves(bitboard:u64, empty:u64, opponents:u64, opponents_pawns:u64, history:Vec<definitions::Move>) -> Vec<definitions::Move> {
    let mut moves: Vec<definitions::Move> = Vec::new();

    //    Capture to the left
    let cap_left = (bitboard << 7) & opponents & !definitions::FILE_H;
    ...
    for i in start..end {
        ...
        if digit & cap_left & !definitions::RANK_8 > 0{
            moves.push(definitions::Move::from_num((i - 7) as u8, i as u8));
        }

            else if digit & cap_left & definitions::RANK_8 > 0 {
    ...
    let cap_right = (bitboard << 9) & opponents & !definitions::FILE_A;

    let end = 64 - cap_right.leading_zeros();
    let start = cap_right.trailing_zeros();

    for i in start..end {
        let digit = 1u64 << i;

        if digit & cap_right & !definitions::RANK_8 > 0 {
            moves.push(definitions::Move::from_num((i - 9) as u8, i as u8));
        }
            else if digit & cap_right & definitions::RANK_8 > 0 {
    ...
    for i in start..end {
        ...
        if digit & forward_1 & !definitions::RANK_8 > 0 {
            moves.push(definitions::Move::from_num((i - 8) as u8, i as u8));
        } else if digit & forward_1 & definitions::RANK_8 > 0 {
    ...
    //    Move forward 2
    let forward_2 = (bitboard << 16) & empty & (empty << 8) & definitions::RANK_4;
